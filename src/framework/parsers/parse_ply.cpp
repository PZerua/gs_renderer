...
bool parse_ply(const char* ply_path, std::vector<Node*>& entities)
{
    ...
    uint32_t vertex_count = ply_file.getElement("vertex").count;
    ...
    for (int i = 0; i < vertex_count; ++i)
    {
        ...
        glm::vec3 color =    { (0.5 + SH_C0 * f_dc_0[i]), (0.5 + SH_C0 * f_dc_1[i]), (0.5 + SH_C0 * f_dc_2[i]) };
        glm::vec3 scale =    { exp(scale0[i]), exp(scale1[i]), exp(scale2[i]) };
        float opacity =      { (1.0f / (1.0f + exp(-opacity_v[i]))) };

        gs_data.push_back({ position, rotation, color, scale, opacity });
    }

    gs_node->create_gs_data_buffer(gs_data);
